...
// ==========================================================================
//  INTERNAL HELPER FUNCTIONS (COMMON FOR BOTH SERIAL & OMP)
// ==========================================================================
...
void find_input_img_coords(Keypoint& kp, float offset_s, float offset_x, float offset_y, float sigma_min=SIGMA_MIN, float min_pix_dist=MIN_PIX_DIST, int n_spo=N_SPO)
{
...
bool refine_or_discard_keypoint(Keypoint& kp, const std::vector<Image>& octave, float contrast_thresh, float edge_thresh)
{
    int k = 0;
    bool kp_is_valid = false; 
    while (k++ < MAX_REFINEMENT_ITERS) {
        auto [offset_s, offset_x, offset_y] = fit_quadratic(kp, octave, kp.scale);
        float max_offset = std::max({std::abs(offset_s), std::abs(offset_x), std::abs(offset_y)});
        
        kp.scale += std::round(offset_s);
        kp.i += std::round(offset_x);
        kp.j += std::round(offset_y);
        
        if (kp.scale >= (int)octave.size()-1 || kp.scale < 1) break;
...
void smooth_histogram(float hist[N_BINS])
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        for (int j = 0; j < N_BINS; j++) hist[j] = tmp_hist[j];
    }
}

std::vector<float> find_keypoint_orientations(Keypoint& kp, const ScaleSpacePyramid& grad_pyramid, float lambda_ori, float lambda_desc)
{
    ...
    if (min_dist_from_border <= std::sqrt(2)*lambda_desc*kp.sigma) return {};
    ...
    for (int x = x_start; x <= x_end; x++) {
        for (int y = y_start; y <= y_end; y++) {
            ...
            float weight = std::exp(-(std::pow(x*pix_dist-kp.x, 2)+std::pow(y*pix_dist-kp.y, 2)) / (2*patch_sigma*patch_sigma));
    ...
    for (int j = 0; j < N_BINS; j++) if (hist[j] > ori_max) ori_max = hist[j];
...
void hists_to_vec(float histograms[N_HIST][N_HIST][N_ORI], std::array<uint8_t, 128>& feature_vec)
{
    ...
    float norm = 0;
    for (int i = 0; i < size; i++) norm += hist[i] * hist[i];
    norm = std::sqrt(norm);
    float norm2 = 0;
...
void compute_keypoint_descriptor(Keypoint& kp, float theta, const ScaleSpacePyramid& grad_pyramid, float lambda_desc)
{
    ...
    for (int m = x_start; m <= x_end; m++) {
        for (int n = y_start; n <= y_end; n++) {
            ...
            if (std::max(std::abs(x), std::abs(y)) > lambda_desc*(N_HIST+1.)/N_HIST) continue;
            ...
            update_histograms(histograms, x, y, weight * grad_norm, theta_mn, lambda_desc);
        }
    }
...
ScaleSpacePyramid generate_gaussian_pyramid_serial(const Image& img, float sigma_min, int num_octaves, int scales_per_octave) {
    float base_sigma = sigma_min / MIN_PIX_DIST;
    Image base_img = img.resize_serial(img.width*2, img.height*2, Interpolation::BILINEAR);
    float sigma_diff = std::sqrt(base_sigma*base_sigma - 1.0f);
    base_img = gaussian_blur_serial(base_img, sigma_diff);
    ...
    for (int i = 0; i < num_octaves; i++) {
        ...
        for (int j = 1; j < sigma_vals.size(); j++) {
            ...
            pyramid.octaves[i].push_back(gaussian_blur_serial(prev_img, sigma_vals[j]));
        }
        const Image& next_base_img = pyramid.octaves[i][imgs_per_octave-3];
        base_img = next_base_img.resize_serial(next_base_img.width/2, next_base_img.height/2, Interpolation::NEAREST);
    }
    return pyramid;
}

ScaleSpacePyramid generate_dog_pyramid_serial(const ScaleSpacePyramid& img_pyramid) {
...
std::vector<Keypoint> find_keypoints_serial(const ScaleSpacePyramid& dog_pyramid, float contrast_thresh, float edge_thresh) {
        ...
        for (int j = 1; j < dog_pyramid.imgs_per_octave-1; j++) {
            ...
            for (int x = 1; x < img.width-1; x++) {
                for (int y = 1; y < img.height-1; y++) {
                    if (std::abs(img.get_pixel(x, y, 0)) < 0.8f * contrast_thresh) continue;
...
ScaleSpacePyramid generate_gradient_pyramid_serial(const ScaleSpacePyramid& pyramid) {
        ...
        for (int j = 0; j < pyramid.imgs_per_octave; j++) {
            ...
            for (int x = 1; x < grad.width-1; x++) {
                for (int y = 1; y < grad.height-1; y++) {
                    float gx = (pyramid.octaves[i][j].get_pixel(x+1, y, 0) - pyramid.octaves[i][j].get_pixel(x-1, y, 0)) * 0.5f;
                    grad.set_pixel(x, y, 0, gx);
                    float gy = (pyramid.octaves[i][j].get_pixel(x, y+1, 0) - pyramid.octaves[i][j].get_pixel(x, y-1, 0)) * 0.5f;
...
std::vector<Keypoint> find_keypoints_and_descriptors_serial(const Image& img, float sigma_min, int num_octaves, int scales_per_octave, float contrast_thresh, float edge_thresh, float lambda_ori, float lambda_desc)
{
    assert(img.channels == 1 || img.channels == 3);
    const Image& input = img.channels == 1 ? img : rgb_to_grayscale_serial(img);
    
    ScaleSpacePyramid gaussian_pyramid = generate_gaussian_pyramid_serial(input, sigma_min, num_octaves, scales_per_octave);
    ScaleSpacePyramid dog_pyramid = generate_dog_pyramid_serial(gaussian_pyramid);
    std::vector<Keypoint> tmp_kps = find_keypoints_serial(dog_pyramid, contrast_thresh, edge_thresh);
    ScaleSpacePyramid grad_pyramid = generate_gradient_pyramid_serial(gaussian_pyramid);
...
Image draw_matches_serial(const Image& a, const Image& b, std::vector<Keypoint>& kps_a, std::vector<Keypoint>& kps_b, std::vector<std::pair<int, int>> matches)
{
...
ScaleSpacePyramid generate_gaussian_pyramid_omp(const Image& img, float sigma_min, int num_octaves, int scales_per_octave) {
    float base_sigma = sigma_min / MIN_PIX_DIST;
    Image base_img = img.resize_omp(img.width*2, img.height*2, Interpolation::BILINEAR);
    float sigma_diff = std::sqrt(base_sigma*base_sigma - 1.0f);
    base_img = gaussian_blur_omp(base_img, sigma_diff);
    ...
    for (int i = 0; i < num_octaves; i++) {
        ...
        for (int j = 1; j < (int)sigma_vals.size(); j++) {
            const Image& prev_img = pyramid.octaves[i].back();
            pyramid.octaves[i].push_back(gaussian_blur_omp(prev_img, sigma_vals[j]));
        }
        const Image& next_base_img = pyramid.octaves[i][imgs_per_octave-3];
        base_img = next_base_img.resize_omp(next_base_img.width/2, next_base_img.height/2, Interpolation::NEAREST);
    }
    return pyramid;
}

ScaleSpacePyramid generate_dog_pyramid_omp(const ScaleSpacePyramid& img_pyramid) {
    ScaleSpacePyramid dog_pyramid = {
        img_pyramid.num_octaves, img_pyramid.imgs_per_octave - 1,
        std::vector<std::vector<Image>>(img_pyramid.num_octaves)
    };
...
std::vector<Keypoint> find_keypoints_omp(const ScaleSpacePyramid& dog_pyramid, float contrast_thresh, float edge_thresh) {
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp critical
        {
            keypoints.insert(keypoints.end(), local_kps.begin(), local_kps.end());
        }
    }
    return keypoints;
}
...
std::vector<Keypoint> find_keypoints_and_descriptors_omp(const Image& img, float sigma_min, int num_octaves, int scales_per_octave, float contrast_thresh, float edge_thresh, float lambda_ori, float lambda_desc)
{
    assert(img.channels == 1 || img.channels == 3);
    const Image& input = img.channels == 1 ? img : rgb_to_grayscale_omp(img);
    
    ScaleSpacePyramid gaussian_pyramid = generate_gaussian_pyramid_omp(input, sigma_min, num_octaves, scales_per_octave);
    ScaleSpacePyramid dog_pyramid = generate_dog_pyramid_omp(gaussian_pyramid);
    ...
    #pragma omp parallel
    {
        ...
        for (int idx = 0; idx < (int)tmp_kps.size(); ++idx) {
...
std::vector<std::pair<int, int>> find_keypoint_matches_omp(std::vector<Keypoint>& a, std::vector<Keypoint>& b, float thresh_relative, float thresh_absolute)
{
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < (int)a.size(); i++) {
            ...
            if (nn1_dist < thresh_relative * nn2_dist && nn1_dist < thresh_absolute) {
                local_matches.emplace_back(i, nn1_idx);
            }
        }
        #pragma omp critical
        {
...
Image draw_matches_omp(const Image& a, const Image& b, std::vector<Keypoint>& kps_a, std::vector<Keypoint>& kps_b, std::vector<std::pair<int, int>> matches)
{
    Image res(a.width + b.width, std::max(a.height, b.height), 3);
    #pragma omp parallel for
    for (int i = 0; i < a.width; i++) {
