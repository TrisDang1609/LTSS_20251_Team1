...
ScaleSpacePyramid generate_dog_pyramid(const ScaleSpacePyramid& img_pyramid)
{
    ...
    for (int i = 0; i < dog_pyramid.num_octaves; i++) {
        ...
        for (int j = 1; j < img_pyramid.imgs_per_octave; j++) {
            ...
            // song song hóa vòng lặp trên từng pixel
                ...
                for (int x = 1; x < img.width-1; x++) {
                    for (int y = 1; y < img.height-1; y++) {
                        ...
                        if (point_is_extremum(octave, j, x, y)) {
                            Keypoint kp = {x, y, i, j, -1, -1, -1, -1};
                            bool kp_is_valid =
                                refine_or_discard_keypoint(kp, octave,
                                                           contrast_thresh,
                                                           edge_thresh);
                            if (kp_is_valid) {
...
ScaleSpacePyramid generate_gradient_pyramid(const ScaleSpacePyramid& pyramid)
{
    ...
    for (int i = 0; i < pyramid.num_octaves; i++) {
        ...
        for (int j = 0; j < pyramid.imgs_per_octave; j++) {
            ...
            // song song theo trục x, mỗi thread xử lý một số cột
            #pragma omp parallel for
            for (int x = 1; x < grad.width - 1; x++) {
                for (int y = 1; y < grad.height - 1; y++) {
                                                     ...
                                                     float lambda_ori, float lambda_desc)
{
    ...
    ScaleSpacePyramid gaussian_pyramid = generate_gaussian_pyramid(input, sigma_min,
                                                                   num_octaves,
                                                                   scales_per_octave);
    ...
    #pragma omp parallel
    {
        ...
        for (int idx = 0; idx < (int)tmp_kps.size(); ++idx) {
            Keypoint kp_tmp = tmp_kps[idx];  // copy local cho thread

            std::vector<float> orientations =
                find_keypoint_orientations(kp_tmp, grad_pyramid,
                                           lambda_ori, lambda_desc);
...
// float euclidean_dist(std::array<unsigned char, 128>& a,
//                      const std::array<unsigned char, 128>& b)
// {
//     float dist = 0.0f;
                                                       ...
                                                       float thresh_absolute)
{
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < (int)a.size(); i++) {
            // find two nearest neighbours in b for current keypoint from a
            int   nn1_idx  = -1;
            float nn1_dist = 100000000.0f;
            float nn2_dist = 100000000.0f;
            ...
            if (nn1_dist < thresh_relative * nn2_dist &&
                nn1_dist < thresh_absolute) {
...
Image draw_matches(const Image& a, const Image& b,
                   ...
                   std::vector<std::pair<int,int>> matches)
{
    ...
    // phần vẽ line để tuần tự cũng được, số match không quá nhiều
    for (auto &m : matches) {
        Keypoint &kp_a = kps_a[m.first];
        Keypoint &kp_b = kps_b[m.second];
        draw_line(res, kp_a.x, kp_a.y, a.width + kp_b.x, kp_b.y);
