...
namespace sift
{
        ...
        : buffer(nullptr), num_keypoints(n_kps)
    {
        ...
        size_t buffer_size = n_kps * 128 * sizeof(float);
                             ...
                             int descriptor_idx)
    {
        ...
        if (descriptors.buffer && descriptor_idx < descriptors.num_keypoints)
        {
            ...
            std::vector<float> all_descriptors(descriptors.num_keypoints * 128);
            cl_int err = opencl_api.read_buffer(
                descriptors.buffer,
                descriptors.num_keypoints * 128 * sizeof(float),
            ...
            {
                // Extract the specific descriptor and convert float to uint8_t
                size_t offset = descriptor_idx * 128;
                for (int i = 0; i < 128; i++)
                {
                    cpu_kp.descriptor[i] = static_cast<uint8_t>(
                        std::min(255.0f, std::max(0.0f, all_descriptors[offset + i])));
                }
            }
        }
                                                             ...
                                                             float lambda_desc)
    {
        ...
        */
        ...
        for (int oct = 0; oct < num_octaves; oct++)
        {
            ...
            // Get DoG buffers for this octave (scale 1 for prev/curr/next logic)
            cl_mem dog_prev = dog_pyramid.get_buffer(oct, 0);
            cl_mem dog_curr = dog_pyramid.get_buffer(oct, 1);
            cl_mem dog_next = dog_pyramid.get_buffer(oct, 2);

            clSetKernelArg(refine_kernel, 0, sizeof(cl_mem), &candidate_buffer);
            clSetKernelArg(refine_kernel, 1, sizeof(int), &num_candidates);
            ...
            clSetKernelArg(refine_kernel, 8, sizeof(float), &contrast_thresh);
            clSetKernelArg(refine_kernel, 9, sizeof(float), &edge_thresh);
            clSetKernelArg(refine_kernel, 10, sizeof(float), &sigma_min);
            clSetKernelArg(refine_kernel, 11, sizeof(float), &MIN_PIX_DIST);
            clSetKernelArg(refine_kernel, 12, sizeof(int), &scales_per_octave);
            clSetKernelArg(refine_kernel, 13, sizeof(cl_mem), &refined_buffer);
            clSetKernelArg(refine_kernel, 14, sizeof(cl_mem), &refined_counter);
        ...
        const int MAX_ORIENTED = std::min(70000, num_refined * 3); // Cap at 70K or 3× refined
        ...
        for (int oct = 0; oct < num_octaves; oct++)
        {
            ...
            cl_mem grad_buffer = grad_pyramid.get_buffer(oct, 0); // All scales in one buffer

            clSetKernelArg(orientation_kernel, 0, sizeof(cl_mem), &refined_buffer);
            clSetKernelArg(orientation_kernel, 1, sizeof(int), &num_refined);
            ...
            clSetKernelArg(orientation_kernel, 5, sizeof(float), &lambda_ori);
            clSetKernelArg(orientation_kernel, 6, sizeof(float), &lambda_desc);
            clSetKernelArg(orientation_kernel, 7, sizeof(float), &MIN_PIX_DIST);
            clSetKernelArg(orientation_kernel, 8, sizeof(cl_mem), &oriented_buffer);
            clSetKernelArg(orientation_kernel, 9, sizeof(cl_mem), &oriented_counter);
        ...
        cl_mem descriptor_buffer = opencl_api.create_buffer(
            num_oriented * 128 * sizeof(uint8_t),
            CL_MEM_READ_WRITE, nullptr);
        ...
        {
            auto [width, height] = grad_pyramid.get_dimensions(oct);
            cl_mem grad_buffer = grad_pyramid.get_buffer(oct, 0);

            clSetKernelArg(descriptor_kernel, 0, sizeof(cl_mem), &oriented_buffer);
            clSetKernelArg(descriptor_kernel, 1, sizeof(int), &num_oriented);
            ...
            clSetKernelArg(descriptor_kernel, 5, sizeof(float), &lambda_desc);
            clSetKernelArg(descriptor_kernel, 6, sizeof(float), &MIN_PIX_DIST);
            clSetKernelArg(descriptor_kernel, 7, sizeof(cl_mem), &descriptor_buffer);
...
__kernel void find_keypoints(
    __global const float* dog_prev,
    __global const float* dog_curr,
    __global const float* dog_next,
    __global int* candidate_buffer,
    __global int* candidate_counter,
    int width, int height, int octave, int scale,
